...
namespace fPotencia {
	 ...
	 */
	class CarsonEquations final {
	public:
		CarsonEquations(double frequency, double earth_resistivity,
		                double air_permitivity);
	...
	public:
		 ...
		 */
		Conductor(std::string name, double geometric_mean_radius,
		          double resistance, double diameter, double capacity,
	...
	public:
		 ...
		 */
		UndergroundCable(Conductor phase_conductor, Conductor neutral_conductor,
		 ...
		 * earth_resistivity: earth resistivity (in ohm-m, a ussual value is
		 * 100)
		 */
		UndergroundCable(Conductor phase_conductor, double tape_diameter,
	...
	private:
		...
		double Eps_r; // relative permitivity of the medium between the phase
		              // condutor and the neutrals
	...
	public:
		 ...
		 * x: x position (in ft)
		 * y: y position (in ft)
		 * these are compared to the GMR therefore the GMR must be in the same
		 * units
		 */
		void addCable(UndergroundCable cbl, ConnectionPhase phase, double x,
		              double y);
	...
	public:
		...
		void addConductor(Conductor conductor, ConnectionPhase phase, double x,
		                  double y);
	...
	public:
		...
		Ditch(double frequency, double earth_resistivity,
		      double air_permitivity, Units_mode unit_mode);
		...
		std::vector<UndergroundLine> lines;

		// phases of the cables
		std::vector<Conductor> phases;
		std::vector<cx_double> phases_pos;

		// neutrals of the cables
		std::vector<Conductor> neutrals;
		std::vector<cx_double> neutrals_pos;

		// extra neutrals
		std::vector<Conductor> common_neutrals;
		std::vector<cx_double> common_neutrals_pos;

		// all the conductors and their positions ordered
		std::vector<Conductor> conductors;
		std::vector<cx_double> conductors_pos;

		std::vector<cx_double> C_shunt;
	...
	public:
		...
		Tower(double frequency, double earth_resistivity,
		      double air_permitivity, Units_mode unit_mode);
