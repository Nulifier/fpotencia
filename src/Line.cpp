...
namespace fPotencia {
	LineType::LineType(const std::string& name, double r, double x, double b,
	...
	Line::Line(const std::string& name, int connection_bus1,
	           int connection_bus2, const LineType& line_type,
	           double line_length)
	    : Name(name), bus1(connection_bus1), bus2(connection_bus2),
	      length(line_length) {
		SetType(line_type);
	}

	/*
	 * This function calculates the line impedance and admittance from a line
	 * type model
	 */
	void Line::SetType(const LineType& line_type) {
	...
	void Line::print() const {
		...
		std::cout << "\t r:" << impedance.real() << ", x:" << impedance.imag()
		          << ", c: " << shunt_admittance.imag() << std::endl;
